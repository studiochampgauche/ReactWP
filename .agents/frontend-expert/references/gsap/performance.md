...
Use **will-change** only on the exact element whose upcoming transform/opacity work benefits from promotion. Confirm the benefit in the target browser; each promoted layer consumes memory.

```css
will-change: transform;
```

Pinning needs a separate decision. A ScrollTrigger pin may use `position: fixed` or transforms. For a confirmed transform-driven pin or animated child, `will-change: transform` on that exact moving layer may reduce shake. Do not apply it universally, and never place it on an ancestor of a fixed pin: browsers treat it like an applied transform for containing-block behavior, which can break fixed positioning. Read `scrolltrigger.md` and verify `pinType`, ancestor transforms, the actual scroller, and rapid-scroll behavior before keeping the hint.
...
- A shaking pin is not proof that it lacks `will-change`. Determine whether pinning is fixed or transform-based and fix scroller synchronization, ancestor containing blocks, pin mode, refresh order, or layout instability first.
...
- ✅ Animate **transform** and **opacity**; use **will-change** only on a measured transform/opacity hot layer and apply the ScrollTrigger pin-mode rules above.
...
- ❌ Treat `will-change: transform` as mandatory for every pin or put it on an ancestor of a fixed pin.
