...
- for a redesign governed by a user-feedback ledger, audit the rendered result against every `keep`, `remove`, `rebuild`, and `add` row. Reintroducing a rejected structural family under new selectors, copy, colors, radii, or motion is a failure;
